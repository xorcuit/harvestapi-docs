...
```bash cURL
curl --request GET \
  --url https://api.harvestapi.io/linkedin/post-search?search=Software&postedLimit=24h&sortBy=relevance&page=1 \
...
```bash cURL
curl --request GET \
  --url https://api.harvestapi.io/linkedin/post-search?authorsCompany=https://www.linkedin.com/company/google \
  --header 'X-API-Key: <api-key>'
```

...
const firstPage = await fetch(
  `https://api.harvestapi.io/linkedin/post-search?${params.toString()}`,
  {
...
```bash cURL
curl --request GET \
  --url https://api.harvestapi.io/linkedin/post-search?profileId=ACoAAA8BYqEBCGLg_vT_ca6mMEqkpp9nVffJ3hc&postedLimit=month&sortBy=date&page=1 \
